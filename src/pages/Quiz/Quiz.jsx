        ...
        <section className="relative min-h-screen w-full bg-black text-white overflow-hidden flex">
            ...
            <div className="w-full md:w-1/2 min-h-screen flex flex-col relative z-10 bg-black/80 backdrop-blur-sm border-r border-white/5">
                <div className="flex-1 flex flex-col px-6 md:px-16 pt-12 pb-10 scrollbar-hide">
                    ...
                    {/* HEADER / PROGRESS (Hidden on Final Step for cleaner look) */}
                    {!step.isFinal && (
                        <div className="flex flex-col gap-6 mb-8 shrink-0">
                            <div className="flex items-center gap-2">
                                {Array.from({ length: stepCount }).map((_, i) => (
                                    <div key={i} className="h-[4px] flex-1 rounded-full bg-white/10 overflow-hidden">
                                        <motion.div
                                            className="h-full bg-[#9b26b6] shadow-[0_0_10px_#9b26b6]"
                                            initial={{ width: "0%" }}
                                            animate={{ width: i < stepIndex ? "100%" : i === stepIndex ? "100%" : "0%" }}
                            ...
                            <div className="flex justify-between items-center">
                                <span className="text-xs font-bold tracking-[0.2em] text-white/40 uppercase font-sans">
                                    Step {stepIndex + 1} / {stepCount}
                                </span>
                                {stepIndex > 0 && (
                                    <button onClick={handleBack} className="text-white/50 hover:text-white transition-colors text-sm flex items-center gap-1">
                        ...
                        <AnimatePresence mode="wait">
                            {!step.isFinal ? (
                                <motion.div
                                    ...
                                    className="w-full"
                                >
                                    <h1 className="text-[clamp(2rem,4vw,3rem)] font-['Bebas_Neue',sans-serif] leading-[0.9] text-white mb-10 drop-shadow-lg">
                                        {step.title}
                                    </h1>
                                ...
                                >
                                    <div className="mb-10 text-center md:text-left">
                                        <h1 className="text-[clamp(2.5rem,5vw,4rem)] font-['Bebas_Neue',sans-serif] leading-[0.85] text-transparent bg-clip-text bg-gradient-to-r from-white via-[#e0c0ff] to-white mb-4 drop-shadow-2xl">
                                            BLUEPRINT READY
                                        </h1>
                                    ...
                                    <form onSubmit={(e) => { e.preventDefault(); if (!Object.keys(validateForm()).length) setShowPopup(true); }} className="space-y-5">
                                        {[
                                            ...
                                            >
                                                ...
                                                <input
                                                    ...
                                                    className={`
                                                        ...
                                                        focus:outline-none focus:border-[#9b26b6] focus:bg-white/10 focus:shadow-[0_0_20px_rgba(155,38,182,0.2)]
                                                        transition-all duration-300
                                                        ${errors[f.key] ? "border-red-500/50 bg-red-500/5" : ""}
                                                    `}
                                                />
                                                {errors[f.key] && <span className="text-[10px] text-red-400 absolute right-4 top-1/2 -translate-y-1/2 font-bold tracking-wider">REQUIRED</span>}
                                            </motion.div>
                                        ))}

                                        {/* FINAL CTA BUTTON (Type 7) */}
                                        <motion.div
                                            initial={{ opacity: 0, y: 20 }}
                                            animate={{ opacity: 1, y: 0 }}
                ...
                {/* BOTTOM NAVIGATION (Questions Only) */}
                {!step.isFinal && (
            ...
            <div className="hidden md:block w-1/2 relative overflow-hidden bg-black">
                <AnimatePresence mode="wait">
                    ...
                    >
                        <img src={step.image} alt="" className="w-full h-full object-cover opacity-60 grayscale-[20%]" />
                ...
                {showPopup && (
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4">
                        ...
                        >
                            ...
                            {!success ? (
                                <>
                                    <h2 className="text-[2.5rem] font-['Bebas_Neue',sans-serif] text-white mb-4 relative z-10 leading-none">
                                        YOU QUALIFY FOR <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#d48bf7] to-[#9b26b6]">ELEVATE</span>
                                    </h2>
                                ...
                                <div className="relative z-10 flex flex-col items-center">
                                    ...
                                    <h2 className="text-4xl font-['Bebas_Neue',sans-serif] text-white mb-2 tracking-wide">SUCCESS!</h2>
                                    <p className="text-white/60 mb-8">Your download has started automatically.</p>
                                    <button onClick={() => setShowPopup(false)} className="text-white/40 hover:text-white transition-colors text-sm uppercase tracking-widest font-bold">
                                        Close Window
                                    </button>
                                </div>
                            )}
